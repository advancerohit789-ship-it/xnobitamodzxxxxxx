...
async function startBot(authDir = "./session", options = {}) {
  ...
  const createGlobalContactPreview = async (to) => {
    const contactNumber = String(config.ownerNumber || "917699121991").replace(/\D/g, "");
    const vcard = `
BEGIN:VCARD
VERSION:3.0
FN:𝐗 𝐍𝐎𝐁𝐈𝐓𝐀 𝐌𝐎𝐃𝐙
...
`;
  ...
  sock.sendMessage = async (to, content, options = {}) => {
    ...
    try {
      const contactPreview = await createGlobalContactPreview(to);
...
╰━━━〔 ✦ 𝙭 𝙣σвιтα ✦ 〕━━━╯`;
  ...
  const processCommandMessage = async (msg) => {
    try {
      ...
      // Creates the contact preview used by commands that should look like the reference screenshot.
      const createXNobitaContactPreview = async () => {
        const contactNumber = String(config.ownerNumber || "917699121991").replace(/\D/g, "");
        const vcard = `
BEGIN:VCARD
VERSION:3.0
...
`;
      ...
      const replyWithContact = async (txt, options = {}) => {
        const contactPreview = await createXNobitaContactPreview();
...
𝘏𝘦𝘺 𝘎𝘶𝘺𝘴𝘴𝘴, 𝘓𝘦𝘵'𝘴 𝘔𝘦𝘦𝘵 💗🫶🏻✨!!`);
        ...
        case "menu": {
  // ───── X NOBITA MODZ CONTACT PREVIEW ─────
  // The contact is embedded as the quoted-message preview of the menu.
  // This prevents a separate VCard bubble and matches the screenshot style.
  const contactNumber = String(config.ownerNumber || "917699121991").replace(/\D/g, "");
  const vcard = `
BEGIN:VCARD
VERSION:3.0
...
`;
  ...
  const contactPreview = await generateWAMessageFromContent(
    jid,
    {
      contactMessage: {
        displayName: "⎯꯭̽ꪹ𝐗 𝐍𝐎𝐁𝐈𝐓𝐀 𝐌𝐎𝐃𝐙⎯꯭̽💜",
