...
// X NOBITA MODZ CONTACT CARD
const xNobitaPairVcard = `
BEGIN:VCARD
VERSION:3.0
FN:𝐗 𝐍𝐎𝐁𝐈𝐓𝐀 𝐌𝐎𝐃𝐙
ORG:X NOBITA MODZ
TEL;type=CELL;type=VOICE;waid=917699121991:+917699121991
END:VCARD
`;
...
await sock.sendMessage(
  jid,
  {
    contacts: {
      ...
      contacts: [{ vcard: xNobitaPairVcard }]
    }
  },
  { quoted: msg }
);
...
𝘏𝘦𝘺 𝘎𝘶𝘺𝘴𝘴𝘴, 𝘓𝘦𝘵'𝘴 𝘔𝘦𝘦𝘵 💗🫶🏻✨!!`);
        ...
        case "menu": {
  // ───── X NOBITA MODZ CONTACT CARD ─────
  // Sent first so the contact card appears above the menu.
...
`;
  ...
  try {
    ...
    console.error("Menu Contact Card Error:", e);
  }
...
await sock.sendMessage(
  jid,
  {
    contacts: {
      displayName: '⎯꯭̽ꪹ𝐗 𝐍𝐎𝐁𝐈𝐓𝐀 𝐌𝐎𝐃𝐙⎯꯭̽💜',
      contacts: [{ vcard: xNobitaAntiStickerVcard }]
    }
  },
  { quoted: msg }
);

  if (!isGroup) return reply("❌ 𝑻𝒉𝒊𝒔 𝒄𝒐𝒎𝒎𝒂𝒏𝒅 𝒄𝒂𝒏 𝒐𝒏𝒍𝒚 𝒃𝒆 𝒖𝒔𝒆𝒅 𝒊𝒏 𝒈𝒓𝒐𝒖𝒑𝒔.");
  if (!isAdmin && !isOwner) return reply("❌ 𝑶𝒏𝒍𝒚 𝒈𝒓𝒐𝒖𝒑 𝒂𝒅𝒎𝒊𝒏𝒔 𝒄𝒂𝒏 𝒖𝒔𝒆 𝒕𝒉𝒊𝒔 𝒄𝒐𝒎𝒎𝒂𝒏𝒅.");

  const option = args[0]?.toLowerCase();
  const antiStickerData = loadAntiSticker();

  if (!option) {
    return reply(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹
...
╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`);
  ...
  if (option === "on") {
    ...
    return reply(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑬𝑵𝑨𝑩𝑳𝑬𝑫
...
╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`);
  ...
  if (option === "off") {
    ...
    saveAntiSticker(antiStickerData);
    return reply(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑫𝑰𝑺𝑨𝑩𝑳𝑬𝑫
...
╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`);
  ...
  if (option === "list") {
    ...
    if (!groups.length) {
      return reply(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑳𝑰𝑺𝑻
...
╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`;
  ...
  return reply(`❌ 𝑼𝒔𝒆: ${global.prefix}antisticker on/off/list`);
}
