...
const ANTISTICKER_FILE = path.join(DATA_DIR, "antisticker.json");
if (!fs.existsSync(ANTISTICKER_FILE)) fs.writeFileSync(ANTISTICKER_FILE, "{}");

function loadAntiSticker() {
  try {
    return JSON.parse(fs.readFileSync(ANTISTICKER_FILE, "utf8"));
  } catch {
    return {};
  }
}
...
function saveAntiSticker(data) {
  fs.writeFileSync(ANTISTICKER_FILE, JSON.stringify(data, null, 2), "utf8");
}
...
async function startBot(authDir = "./session", options = {}) {
  ...
  // ───── GLOBAL X NOBITA MODZ CONTACT PREVIEW ─────
  // Every normal outgoing bot message gets the X NOBITA MODZ contact
  // as its quoted preview. Contact messages/reactions are left untouched
  // to avoid recursion or duplicate contact cards.
  const originalSendMessage = sock.sendMessage.bind(sock);

  // Shared WhatsApp Channel forwarding metadata for Menu and Connected DM notice.
  const contextInfo = {
    isForwarded: true,
    forwardingScore: 999,
  ...
  const createGlobalContactPreview = async (to, quotedMessage = null) => {
    const contactNumber = "917699121991";
    const contactName = "х noвιta мod";
    const vcard = [
      "BEGIN:VCARD",
      "VERSION:3.0",
      `N:;${contactName};;;`,
    ...
    // Use the command sender's WhatsApp profile photo for the small DP
    // shown on the quoted Miku-style contact card.
    const senderJid = quotedMessage?.key?.participant || quotedMessage?.key?.remoteJid || null;
    const senderPhoto = senderJid ? await getProfilePictureBuffer(sock, senderJid) : null;

    const contactMessage = {
      displayName: contactName,
      vcard
    };
    if (senderPhoto) contactMessage.jpegThumbnail = senderPhoto;

    return {
      key: {
        fromMe: false,
        participant: `${contactNumber}@s.whatsapp.net`,
  ...
  sock.sendMessage = async (to, content, options = {}) => {
    const isReaction = !!content?.react;
    const isContact = !!content?.contacts || !!content?.contactMessage;
    const isProtocol = !!content?.protocolMessage;
    const alreadyContactQuoted = !!options?.quoted?.message?.contactMessage;

    if (isReaction || isContact || isProtocol || alreadyContactQuoted) {
      return originalSendMessage(to, content, options);
    }

    try {
      const contactPreview = await createGlobalContactPreview(to, options?.quoted);
      return originalSendMessage(to, content, {
        ...options,
        quoted: contactPreview
      });
    } catch (e) {
      console.error("Global Contact Preview Error:", e?.message || e);
      return originalSendMessage(to, content, options);
    }
  };
...
╰━━━〔 ✦ 𝙭 𝙣σвιтα ✦ 〕━━━╯`;
        ...
        await sock.sendMessage(ownerJid, { text: successMsg, contextInfo });
  ...
  sock.ev.on("group-participants.update", async (update) => {
    try {
      ...
      const settingsWelcome = botSettings.group(botJid, "welcome");
      const settingsGoodbye = botSettings.group(botJid, "goodbye");
  ...
  const processCommandMessage = async (msg) => {
    try {
      ...
      const botJidForSettings = (sock.user?.id || options.pairedPhone || "").split(":")[0];
      const currentBotSettings = botSettings.get(botJidForSettings);
      if (isGroup && currentBotSettings.groups.antigm[jid]) {
      ...
      // --- ANTISTICKER AUTO DELETE ---
      const antiStickerData = botSettings.group(botJidForSettings, "antisticker");
      if (isGroup && antiStickerData[jid] === true && msg.message?.stickerMessage) {
        try {
          await sock.sendMessage(jid, {
            delete: {
              remoteJid: jid,
          ...
          console.error("AntiSticker Delete Error:", e?.message || e);
        }
        return;
      }
      ...
      await antiLinkHandler(sock, msg, botJidForSettings);

      const prefix = currentBotSettings.prefix || config.prefix;
      ...
      // Normal command reply. The global sendMessage wrapper adds the single X NOBITA MODZ vCard preview.
      const replyWithContact = async (txt, options = {}) => {
        await sock.sendMessage(jid, { text: txt, ...options });
      };
      ...
      const botJid = botJidForSettings;
      ...
      if (settings.botMode === "private" && !isOwner) return;
      ...
      switch (command) {
        case "pair": {
          ...
          if (!rawNumber) {
            await replyWithContact(`╭━━━〔 🔐 𝙇𝙄𝙉𝙆 𝘿𝙀𝙑𝙄𝘾𝙀 〕━━━╮
┃
┃ 📱 𝙐𝙨𝙖𝙜𝙚:
┃   .pair 91XXXXXXXXXX
┃
...
╰━━━━━━━━━━━━━━━━━━━━╯`);
          ...
          if (!/^\d{8,15}$/.test(phone)) {
            await replyWithContact(`❌ 𝙄𝙣𝙫𝙖𝙡𝙞𝙙 𝙥𝙝𝙤𝙣𝙚 𝙣𝙪𝙢𝙗𝙚𝙧!\n\n📱 Example: .pair 919876543210`);
            break;
          }
          ...
          if (pairStatus === "starting") {
            await replyWithContact(`⏳ 𝙋𝙖𝙞𝙧𝙞𝙣𝙜 𝙞𝙨 𝙖𝙡𝙧𝙚𝙖𝙙𝙮 𝙞𝙣 𝙥𝙧𝙤𝙜𝙧𝙚𝙨𝙨 𝙛𝙤𝙧 +${phone}.`);
            break;
          }

          if (sessionManager.isRunning(phone)) {
            await replyWithContact(`🟢 +${phone} 𝙞𝙨 𝙖𝙡𝙧𝙚𝙖𝙙𝙮 𝙘𝙤𝙣𝙣𝙚𝙘𝙩𝙚𝙙 𝙩𝙤 𝙓 𝙉𝙊𝘽𝙄𝙏𝘼 𝙈𝙊𝘿𝙕.`);
            break;
          }

          await replyWithContact(`╭━━━〔 💞 𝙓 𝙉𝙊𝘽𝙄𝙏𝘼 𝙈𝙊𝘿𝙕 💞 〕━━━╮
┃
┃ 📱 𝙉𝙪𝙢𝙗𝙚𝙧: +${phone}
┃
...
╰━━━━━━━━━━━━━━━━━━━━╯`);
            ...
            await replyWithContact(`❌ 𝙋𝘼𝙄𝙍𝙄𝙉𝙂 𝙀𝙍𝙍𝙊𝙍!\n\n${err?.message || "Unable to start WhatsApp pairing."}`);
          }

          break;
        }
          case "misti":
           await replyWithContact(`𝘏𝘦𝘦𝘺𝘺𝘺𝘺💋🦋!!
🌸 𝘔𝘪𝘴𝘵𝘪𝘪𝘪 𝘏𝘦𝘳𝘦𝘦𝘦 😋🎀!!
...
𝘏𝘦𝘺 𝘎𝘶𝘺𝘴𝘴𝘴, 𝘓𝘦𝘵'𝘴 𝘔𝘦𝘦𝘵 💗🫶🏻✨!!`);
  ...
  await reply(`* ╰┈➤🎡 ᎮoŇᎶ: ${speed.toFixed(2)} ๓Ş*`);

  break;
        }

        case "alive":
          await replyWithContact(`🤖 Bot : ${config.botName}\n⚡ Status : Online\n🔖 Prefix : ${settings.prefix}\n🕐 Time : ${new Date().toLocaleString()}`);
          break;

        case "owner":
          const ownerMsg = `👑 𝑶𝑾𝑵𝑬𝑹 • 𝙓 𝙉𝑶𝑩𝑰𝑻𝑨 💚\n📱 𝑶𝒘𝒏𝒆𝒓 𝑶𝒇 𝑿 𝙉𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`;
          await replyWithContact(ownerMsg);
          break;

        case "botlink":
          await replyWithContact(`💚 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑩𝑶𝑻\n\n🔗 𝑷𝒂𝒊𝒓 𝑳𝒊𝒏𝒌\n➜ https://t.me/pbv56NuNOhIzZjc9\n\n📲 𝑮𝒐 𝑻𝒐 𝑳𝒊𝒏𝒌 𝑨𝒏𝒅 𝑻𝒚𝒑𝒆\n\n📌 𝑬𝒙𝒂𝒎𝒑𝒍𝒆\n➜ .pair (country code)(number)\n\n🇮🇳 𝑬𝒙𝒂𝒎𝒑𝒍𝒆:\n➜ .pair 919876543210`);
          break;

        case "menu": {
  const uptime = formatTime(process.uptime());
  const mode = settings.botMode.toLowerCase();
...
~ 𝐌𝐚𝐝𝐞 𝐰𝐢𝐭𝐡 𝐥𝐨𝐯𝐞 𝐛𝐲 𝐗 𝐍𝐎𝐁𝐈𝐓𝐀 𝐌𝐎𝐃 🫶🏻❤️‍🩹 ~`;
  ...
  if (config.menuImage) {
    ...
    await sock.sendMessage(jid, { text: menuText, contextInfo });
  }
...
// --- BOT CONTROL ---
case "public":
  if (!isBotNumber) return replyWithContact("❌ 𝐎𝐧𝐥𝐲 𝐛𝐨𝐭 𝐥𝐨𝐠𝐢𝐧 𝐧𝐮𝐦𝐛𝐞𝐫 𝐜𝐚𝐧 𝐮𝐬𝐞 𝐭𝐡𝐢𝐬 𝐜𝐨𝐦𝐦𝐚𝐧𝐝.");
  botSettings.update(botJid, s => { s.botMode = "public"; return s; });
  await replyWithContact(`🌐 𝐏𝐔𝐁𝐋𝐈𝐂 𝐌𝐎𝐃𝐄
...
case "private":
  if (!isBotNumber) return replyWithContact("❌ 𝐎𝐧𝐥𝐲 𝐛𝐨𝐭 𝐥𝐨𝐠𝐢𝐧 𝐧𝐮𝐦𝐛𝐞𝐫 𝐜𝐚𝐧 𝐮𝐬𝐞 𝐭𝐡𝐢𝐬 𝐜𝐨𝐦𝐦𝐚𝐧𝐝.");
  botSettings.update(botJid, s => { s.botMode = "private"; return s; });
  await replyWithContact(`🔒 𝐏𝐑𝐈𝐕𝐀𝐓𝐄 𝐌𝐎𝐃𝐄
...
case "setprefix":
  if (!isBotNumber) return replyWithContact("❌ 𝐎𝐧𝐥𝐲 𝐛𝐨𝐭 𝐥𝐨𝐠𝐢𝐧 𝐧𝐮𝐦𝐛𝐞𝐫 𝐜𝐚𝐧 𝐮𝐬𝐞 𝐭𝐡𝐢𝐬 𝐜𝐨𝐦𝐦𝐚𝐧𝐝.");
  if (!args[0]) return replyWithContact(`⚙️ 𝐏𝐑𝐄𝐅𝐈𝐗 𝐒𝐄𝐓𝐓𝐈𝐍𝐆

🔹 𝐂𝐮𝐫𝐫𝐞𝐧𝐭 𝐏𝐫𝐞𝐟𝐢𝐱: ${settings.prefix}

💡 𝐔𝐬𝐚𝐠𝐞: .setprefix !`);
  if (args[0].length > 3) return replyWithContact("❌ 𝐏𝐫𝐞𝐟𝐢𝐱 𝐦𝐮𝐬𝐭 𝐛𝐞 𝟏–𝟑 𝐜𝐡𝐚𝐫𝐚𝐜𝐭𝐞𝐫𝐬.");
  botSettings.update(botJid, s => { s.prefix = args[0]; return s; });
  await replyWithContact(`⚙️ 𝐏𝐑𝐄𝐅𝐈𝐗 𝐔𝐏𝐃𝐀𝐓𝐄𝐃

🔹 𝐍𝐞𝐰 𝐏𝐫𝐞𝐟𝐢𝐱: ${args[0]}`);
  break;

case "restart":
  if (!isOwner) return replyWithContact("❌ 𝐎𝐰𝐧𝐞𝐫 𝐨𝐧𝐥𝐲 𝐜𝐨𝐦𝐦𝐚𝐧𝐝.");
  await replyWithContact(`🔄 𝐑𝐄𝐒𝐓𝐀𝐑𝐓𝐈𝐍𝐆...
...
// --- UTILITY COMMANDS ---
case "uptime":
  await replyWithContact(`⏱️ 𝐔𝐏𝐓𝐈𝐌𝐄: ${formatTime(process.uptime())}
📡 𝐒𝐭𝐚𝐭𝐮𝐬: 𝐎𝐧𝐥𝐢𝐧𝐞`);
  break;

case "runtime":
  await replyWithContact(`⚡ 𝐑𝐔𝐍𝐓𝐈𝐌𝐄: ${formatTime(process.uptime())}
🤖 𝐁𝐨𝐭: ${config.botName}`);
  break;

case "info":
  await replyWithContact(`👑 𝐎𝐰𝐧𝐞𝐫: ${config.ownerName}
🤖 𝐁𝐨𝐭: ${config.botName}
⚡ 𝐏𝐫𝐞𝐟𝐢𝐱: ${settings.prefix}
💚 𝐌𝐨𝐝𝐞: ${settings.botMode.toUpperCase()}
📡 𝐒𝐭𝐚𝐭𝐮𝐬: 𝐎𝐧𝐥𝐢𝐧𝐞`);
  break;

case "profile":
  await replyWithContact(`👤 𝐍𝐚𝐦𝐞: ${msg.pushName || "User"}
🆔 𝐈𝐃: 𝐇𝐢𝐝𝐝𝐞𝐧`);
  break;

case "groupinfo":
  if (!isGroup) return replyWithContact("❌ 𝐆𝐫𝐨𝐮𝐩 𝐨𝐧𝐥𝐲!");
  try {
    const metadataGI = await sock.groupMetadata(jid);
    await replyWithContact(`👥 𝐆𝐫𝐨𝐮𝐩: ${metadataGI.subject}
👤 𝐌𝐞𝐦𝐛𝐞𝐫𝐬: ${metadataGI.participants.length}
👑 𝐀𝐝𝐦𝐢𝐧𝐬: ${metadataGI.participants.filter(p => p.admin).length}
🆔 𝐆𝐫𝐨𝐮𝐩 𝐈𝐃: 𝐇𝐢𝐝𝐝𝐞𝐧`);
  } catch (e) {
    await replyWithContact("❌ 𝐄𝐫𝐫𝐨𝐫 𝐟𝐞𝐭𝐜𝐡𝐢𝐧𝐠 𝐠𝐫𝐨𝐮𝐩 𝐢𝐧𝐟𝐨.");
  }
  break;

case "id":
  await replyWithContact(`🆔 𝐔𝐬𝐞𝐫 𝐈𝐃: ${sender}
🔐 𝐏𝐫𝐢𝐯𝐚𝐜𝐲: 𝐏𝐫𝐨𝐭𝐞𝐜𝐭𝐞𝐝`);
  break;

case "jid":
  if (!q) {
    return replyWithContact(`🆔 𝐂𝐮𝐫𝐫𝐞𝐧𝐭 𝐉𝐈𝐃: ${jid}
...
💡 𝐔𝐬𝐞 .𝐣𝐢𝐝 <𝐥𝐢𝐧𝐤> 𝐭𝐨 𝐠𝐞𝐭 𝐆𝐫𝐨𝐮𝐩/𝐂𝐡𝐚𝐧𝐧𝐞𝐥 𝐉𝐈𝐃.`);
  ...
  try {
    ...
    if (link.includes("chat.whatsapp.com/")) {
      ...
      await replyWithContact(`👥 𝐆𝐑𝐎𝐔𝐏 𝐉𝐈𝐃
...
🆔 𝐉𝐈𝐃: ${cJid}`);
      ...
      await replyWithContact("❌ 𝐔𝐧𝐬𝐮𝐩𝐩𝐨𝐫𝐭𝐞𝐝 𝐥𝐢𝐧𝐤. 𝐔𝐬𝐞 𝐆𝐫𝐨𝐮𝐩 𝐨𝐫 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 𝐥𝐢𝐧𝐤.");
    }
  } catch (e) {
    await replyWithContact("❌ 𝐄𝐫𝐫𝐨𝐫 𝐟𝐞𝐭𝐜𝐡𝐢𝐧𝐠 𝐉𝐈𝐃. 𝐂𝐡𝐞𝐜𝐤 𝐥𝐢𝐧𝐤.");
  }
  break;

case "cstatus":
  if (!isOwner) return replyWithContact("❌ 𝐎𝐰𝐧𝐞𝐫 𝐨𝐧𝐥𝐲 𝐜𝐨𝐦𝐦𝐚𝐧𝐝.");
  if (!q) return replyWithContact("❌ 𝐔𝐬𝐚𝐠𝐞: .cstatus <channel_jid> <text>");
  ...
  try {
    ...
    await replyWithContact("✅ 𝐂𝐡𝐚𝐧𝐧𝐞𝐥 𝐒𝐭𝐚𝐭𝐮𝐬 𝐏𝐨𝐬𝐭𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲!");
  } catch (e) {
    await replyWithContact("❌ 𝐅𝐚𝐢𝐥𝐞𝐝 𝐭𝐨 𝐩𝐨𝐬𝐭 𝐜𝐡𝐚𝐧𝐧𝐞𝐥 𝐬𝐭𝐚𝐭𝐮𝐬.");
  }
  break;
...
case "quote":
  ...
  if (!quotedQ) return replyWithContact("❌ 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐩𝐥𝐲 𝐭𝐨 𝐚 𝐦𝐞𝐬𝐬𝐚𝐠𝐞.");
...
case "add": {
  if (!isGroup) return replyWithContact("❌ 𝙂𝙧𝙤𝙪𝙥 𝙤𝙣𝙡𝙮!");
  if (!isAdmin && !isOwner) return replyWithContact("❌ 𝘼𝙙𝙢𝙞𝙣 𝙤𝙧 𝙊𝙬𝙣𝙚𝙧 𝙤𝙣𝙡𝙮!");
...
case "admins": {
  ...
  return replyWithContact(out, { mentions });
}
...
case "requests": {
  ...
  try {
    if (typeof sock.groupRequestParticipantsList !== "function")
      return replyWithContact("❌ 𝙅𝙤𝙞𝙣 𝙧𝙚𝙦𝙪𝙚𝙨𝙩𝙨 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙨𝙪𝙥𝙥𝙤𝙧𝙩𝙚𝙙.");

    const reqs = await sock.groupRequestParticipantsList(jid);

    if (!reqs?.length)
      return replyWithContact("🌸 𝙉𝙤 𝙥𝙚𝙣𝙙𝙞𝙣𝙜 𝙟𝙤𝙞𝙣 𝙧𝙚𝙦𝙪𝙚𝙨𝙩𝙨.");
    ...
    return replyWithContact(out, { mentions: reqMentions });
  } catch (e) {
    return replyWithContact(`❌ 𝙁𝙖𝙞𝙡𝙚𝙙\n📝 ${e?.message || ""}`);
  }
}
...
case "reject": {
  ...
  if (typeof sock.groupRequestParticipantsUpdate !== "function")
    return replyWithContact("❌ 𝙅𝙤𝙞𝙣 𝙧𝙚𝙦𝙪𝙚𝙨𝙩 𝙛𝙚𝙖𝙩𝙪𝙧𝙚 𝙣𝙤𝙩 𝙨𝙪𝙥𝙥𝙤𝙧𝙩𝙚𝙙.");
  ...
  try {
    ...
    return replyWithContact(`❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 ${command}\n📝 ${e?.message || ""}`);
  }
}
...
case "disappear": {
  ...
  try {
    ...
    return replyWithContact(`❌ 𝙎𝙚𝙩𝙩𝙞𝙣𝙜 𝙁𝙖𝙞𝙡𝙚𝙙\n📝 ${e?.message || ""}`);
  }
}
...
case "poll": {
  ...
  try {
    ...
    return replyWithContact("💗 𝙋𝙤𝙡𝙡 𝘾𝙧𝙚𝙖𝙩𝙚𝙙 𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮 🌸");
  } catch (e) {
    return replyWithContact(`❌ 𝙋𝙤𝙡𝙡 𝙁𝙖𝙞𝙡𝙚𝙙\n📝 ${e?.message || ""}`);
  }
}
...
case "tagall": {
  ...
  await replyWithContact(tagText, {
...
case "kick": {
  if (!isGroup) return replyWithContact("❌ 𝙂𝙧𝙤𝙪𝙥 𝙤𝙣𝙡𝙮!");
  if (!isAdmin && !isOwner)
    return replyWithContact("❌ 𝘼𝙙𝙢𝙞𝙣 𝙤𝙣𝙡𝙮!");
  ...
  if (!targetK)
    return replyWithContact("💡 𝙍𝙚𝙥𝙡𝙮 𝙤𝙧 𝙏𝙖𝙜 𝙖 𝙪𝙨𝙚𝙧 𝙩𝙤 𝙠𝙞𝙘𝙠!");
  ...
  try {
    ...
    if (!botParticipantK?.admin)
      return replyWithContact("❌ 𝘽𝙤𝙩 𝙢𝙪𝙨𝙩 𝙗𝙚 𝙖𝙣 𝙖𝙙𝙢𝙞𝙣!");

    if (!targetParticipantK)
      return replyWithContact("❌ 𝙐𝙨𝙚𝙧 𝙞𝙨 𝙣𝙤𝙩 𝙞𝙣 𝙩𝙝𝙚 𝙜𝙧𝙤𝙪𝙥!");

    if (normalizeJid(targetParticipantK.id) === botIdK)
      return replyWithContact("❌ 𝙄 𝙘𝙖𝙣'𝙩 𝙠𝙞𝙘𝙠 𝙢𝙮𝙨𝙚𝙡𝙛!");

    if (targetParticipantK.admin)
      return replyWithContact("❌ 𝘾𝙖𝙣'𝙩 𝙠𝙞𝙘𝙠 𝙖𝙣 𝙖𝙙𝙢𝙞𝙣!");
    ...
    return replyWithContact("👢 𝙐𝙨𝙚𝙧 𝙆𝙞𝙘𝙠𝙚𝙙 💗");
  } catch (e) {
    console.error("Kick error:", e);
    return replyWithContact(`❌ 𝙆𝙞𝙘𝙠 𝙁𝙖𝙞𝙡𝙚𝙙\n📝 ${e?.message || ""}`);
  }
}

case "kickall": {
  // Group only + Bot Owner only. Group admins/creator are never kicked.
  if (!isGroup) return replyWithContact("❌ 𝙶𝚛𝚘𝚞𝚙 𝙾𝚗𝚕𝚢 — 𝚃𝚑𝚒𝚜 𝚌𝚘𝚖𝚖𝚊𝚗𝚍 𝚌𝚊𝚗 𝚘𝚗𝚕𝚢 𝚋𝚎 𝚞𝚜𝚎𝚍 𝚒𝚗 𝚐𝚛𝚘𝚞𝚙𝚜.");
  if (!isOwner) return replyWithContact("❌ 𝙰𝚌𝚌𝚎𝚜𝚜 𝙳𝚎𝚗𝚒𝚎𝚍 — 👑 𝙾𝚠𝚗𝚎𝚛 𝙾𝚗𝚕𝚢");

  try {
    const metaKA = groupMeta || await sock.groupMetadata(jid);

    // NEVER run kickall unless the bot is confirmed as a group admin.
    // This is a fail-safe: if the bot JID cannot be matched, no one is removed.
    const botIdsKA = new Set([
      normalizeJid(sock.user?.id || ""),
      normalizeJid(sock.user?.lid || ""),
      normalizeJid(botJidForSettings || "")
    ].filter(Boolean));

    const botParticipantKA = metaKA.participants.find(p =>
      botIdsKA.has(normalizeJid(p.id))
    );

    if (!botParticipantKA) {
      return replyWithContact("❌ 𝙱𝚘𝚝 𝙼𝚎𝚖𝚋𝚎𝚛 𝙽𝚘𝚝 𝙵𝚘𝚞𝚗𝚍 — 𝙺𝙸𝙲𝙺𝙰𝙻𝙻 𝙲𝚊𝚗𝚌𝚎𝚕𝚕𝚎𝚍");
    }

    const botIsAdminKA =
      botParticipantKA.admin === "admin" ||
      botParticipantKA.admin === "superadmin";

    if (!botIsAdminKA) {
      return replyWithContact("❌ 𝙱𝚘𝚝 𝙸𝚜 𝙽𝚘𝚝 𝙰𝚍𝚖𝚒𝚗 — 𝙺𝙸𝙲𝙺𝙰𝙻𝙻 𝙲𝚊𝚗𝚌𝚎𝚕𝚕𝚎𝚍");
    }

    const botIdKA = normalizeJid(botParticipantKA.id);

    // Only non-admin members are eligible. Creator/admins and the bot are skipped.
    const membersKA = metaKA.participants.filter(p => {
      const pid = normalizeJid(p.id);
      const isAdminKA = p.admin === "admin" || p.admin === "superadmin";
      return pid !== botIdKA && !isAdminKA;
    });

    if (!membersKA.length) {
      return replyWithContact("⚠️ 𝙽𝚘 𝙼𝚎𝚖𝚋𝚎𝚛𝚜 𝙵𝚘𝚞𝚗𝚍 — 🛡️ 𝙰𝚍𝚖𝚒𝚗𝚜 𝚂𝚔𝚒𝚙𝚙𝚎𝚍");
    }

    await replyWithContact(`⚡ 𝙺𝙸𝙲𝙺𝙰𝙻𝙻 — 𝚁𝚎𝚖𝚘𝚟𝚒𝚗𝚐 ${membersKA.length}...`);

    // Miku-style ONE ACTION: send the complete normal-member JID list
    // in a single groupParticipantsUpdate() call. No one-by-one loop.
    const targetIdsKA = membersKA.map(p => p.id);
    let removedKA = 0;
    let failedKA = 0;

    try {
      await sock.groupParticipantsUpdate(jid, targetIdsKA, "remove");
      removedKA = targetIdsKA.length;
    } catch (err) {
      failedKA = targetIdsKA.length;
      console.error(`[KICKALL] Bulk remove failed (${targetIdsKA.length} members):`, err?.message || err);
    }

    if (failedKA) {
      return replyWithContact(`⚠️ 𝙺𝙸𝙲𝙺𝙰𝙻𝙻 𝙵𝚒𝚗𝚒𝚜𝚑𝚎𝚍 — 💥 ${removedKA} 𝚁𝚎𝚖𝚘𝚟𝚎𝚍 | ❌ ${failedKA} 𝙵𝚊𝚒𝚕𝚎𝚍`);
    }

    return replyWithContact(`✅ 𝙺𝙸𝙲𝙺𝙰𝙻𝙻 𝙳𝚘𝚗𝚎 — 💥 ${removedKA} 𝚁𝚎𝚖𝚘𝚟𝚎𝚍 | 🛡️ 𝙰𝚍𝚖𝚒𝚗𝚜 𝚂𝚔𝚒𝚙𝚙𝚎𝚍`);
  } catch (e) {
    console.error("Kickall error:", e);
    return replyWithContact(`❌ 𝙺𝙸𝙲𝙺𝙰𝙻𝙻 𝙵𝚊𝚒𝚕𝚎𝚍 — ${e?.message || "Unknown error"}`);
  }
}
...
case "promote":
  ...
  if (!mentionP) return replyWithContact("👤 𝙋𝙡𝙚𝙖𝙨𝙚 𝙩𝙖𝙜 𝙖 𝙪𝙨𝙚𝙧!");

  try {
    await sock.groupParticipantsUpdate(jid, [mentionP], "promote");
    await replyWithContact("👑 𝙐𝙨𝙚𝙧 𝙋𝙧𝙤𝙢𝙤𝙩𝙚𝙙 💗");
  } catch (e) {
    await replyWithContact("❌ 𝙋𝙧𝙤𝙢𝙤𝙩𝙚 𝙁𝙖𝙞𝙡𝙚𝙙!");
  }
  break;
...
case "demote":
  ...
  if (!mentionD) return replyWithContact("👤 𝙋𝙡𝙚𝙖𝙨𝙚 𝙩𝙖𝙜 𝙖𝙣 𝙖𝙙𝙢𝙞𝙣!");

  try {
    await sock.groupParticipantsUpdate(jid, [mentionD], "demote");
    await replyWithContact("🌸 𝙐𝙨𝙚𝙧 𝘿𝙚𝙢𝙤𝙩𝙚𝙙");
  } catch (e) {
    await replyWithContact("❌ 𝘿𝙚𝙢𝙤𝙩𝙚 𝙁𝙖𝙞𝙡𝙚𝙙!");
  }
  break;
...
case "grouplink":
  ...
  try {
    ...
    await replyWithContact(
      `🔗 𝙂𝙍𝙊𝙐𝙋 𝙇𝙄𝙉𝙆\n\n` +
      `https://chat.whatsapp.com/${link}`
    );
  } catch (e) {
    await replyWithContact("❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙜𝙚𝙩 𝙜𝙧𝙤𝙪𝙥 𝙡𝙞𝙣𝙠!");
  }
  break;
...
case "revoke":
  ...
  try {
    ...
    await replyWithContact("🔐 𝙂𝙧𝙤𝙪𝙥 𝙇𝙞𝙣𝙠 𝙍𝙚𝙫𝙤𝙠𝙚𝙙 💗");
  } catch (e) {
    await replyWithContact("❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙧𝙚𝙫𝙤𝙠𝙚 𝙡𝙞𝙣𝙠!");
  }
  break;
  ...
  try {
    ...
    await replyWithContact("🔒 𝙈𝙪𝙩𝙚 𝙊𝙉\n👑 𝙊𝙣𝙡𝙮 𝙖𝙙𝙢𝙞𝙣𝙨 𝙘𝙖𝙣 𝙨𝙚𝙣𝙙 𝙢𝙚𝙨𝙨𝙖𝙜𝙚𝙨.");
  } catch (e) {
    await replyWithContact("❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙢𝙪𝙩𝙚 𝙜𝙧𝙤𝙪𝙥.");
  }
  break;
...
case "unmute":
  ...
  try {
    ...
    await replyWithContact("🔓 𝙈𝙪𝙩𝙚 𝙊𝙁𝙁\n💬 𝙀𝙫𝙚𝙧𝙮𝙤𝙣𝙚 𝙘𝙖𝙣 𝙨𝙚𝙣𝙙 𝙢𝙚𝙨𝙨𝙖𝙜𝙚𝙨.");
  } catch (e) {
    await replyWithContact("❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙪𝙣𝙢𝙪𝙩𝙚 𝙜𝙧𝙤𝙪𝙥.");
  }
  break;
...
case "setname":
  if (!isGroup) return replyWithContact("❌ 𝙂𝙧𝙤𝙪𝙥 𝙤𝙣𝙡𝙮!");
  if (!isAdmin && !isOwner) return replyWithContact("❌ 𝘼𝙙𝙢𝙞𝙣 𝙤𝙧 𝙊𝙬𝙣𝙚𝙧 𝙤𝙣𝙡𝙮!");
  if (!q) return replyWithContact("💡 𝙐𝙨𝙚: .setname 𝙉𝙚𝙬 𝙂𝙧𝙤𝙪𝙥 𝙉𝙖𝙢𝙚");
  try {
    await sock.groupUpdateSubject(jid, q);
    await replyWithContact(`✨ 𝙂𝙧𝙤𝙪𝙥 𝙉𝙖𝙢𝙚 𝙐𝙥𝙙𝙖𝙩𝙚𝙙\n🏷️ ${q}`);
  } catch (e) {
    await replyWithContact("❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙘𝙝𝙖𝙣𝙜𝙚 𝙜𝙧𝙤𝙪𝙥 𝙣𝙖𝙢𝙚.");
  }
  break;
...
case "setdesc":
  if (!isGroup) return replyWithContact("❌ 𝙂𝙧𝙤𝙪𝙥 𝙤𝙣𝙡𝙮!");
  if (!isAdmin && !isOwner) return replyWithContact("❌ 𝘼𝙙𝙢𝙞𝙣 𝙤𝙧 𝙊𝙬𝙣𝙚𝙧 𝙤𝙣𝙡𝙮!");
  if (!q) return replyWithContact("💡 𝙐𝙨𝙚: .setdesc 𝙔𝙤𝙪𝙧 𝘿𝙚𝙨𝙘𝙧𝙞𝙥𝙩𝙞𝙤𝙣");
  try {
    await sock.groupUpdateDescription(jid, q);
    await replyWithContact(`📝 𝙂𝙧𝙤𝙪𝙥 𝘿𝙚𝙨𝙘 𝙐𝙥𝙙𝙖𝙩𝙚𝙙\n💗 ${q}`);
  } catch (e) {
    await replyWithContact("❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙘𝙝𝙖𝙣𝙜𝙚 𝙙𝙚𝙨𝙘𝙧𝙞𝙥𝙩𝙞𝙤𝙣.");
  }
  break;
...
case "del":
  ...
  if (!msg.message?.extendedTextMessage?.contextInfo?.quotedMessage)
    return replyWithContact("💡 𝙍𝙚𝙥𝙡𝙮 𝙩𝙤 𝙖 𝙢𝙚𝙨𝙨𝙖𝙜𝙚 𝙖𝙣𝙙 𝙪𝙨𝙚 .delete");
  try {
    ...
    await replyWithContact("❌ 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙙𝙚𝙡𝙚𝙩𝙚 𝙢𝙚𝙨𝙨𝙖𝙜𝙚.");
  }
  break;

case "antisticker": {

  if (!isGroup) return replyWithContact("❌ 𝑻𝒉𝒊𝒔 𝒄𝒐𝒎𝒎𝒂𝒏𝒅 𝒄𝒂𝒏 𝒐𝒏𝒍𝒚 𝒃𝒆 𝒖𝒔𝒆𝒅 𝒊𝒏 𝒈𝒓𝒐𝒖𝒑𝒔.");
  if (!isAdmin && !isOwner) return replyWithContact("❌ 𝑶𝒏𝒍𝒚 𝒈𝒓𝒐𝒖𝒑 𝒂𝒅𝒎𝒊𝒏𝒔 𝒄𝒂𝒏 𝒖𝒔𝒆 𝒕𝒉𝒊𝒔 𝒄𝒐𝒎𝒎𝒂𝒏𝒅.");

  const option = args[0]?.toLowerCase();
  const antiStickerData = botSettings.group(botJid, "antisticker");

  if (!option) {
    return replyWithContact(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹

🔹 ${settings.prefix}antisticker on
🔹 ${settings.prefix}antisticker off
🔹 ${settings.prefix}antisticker list

╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`);
  }

  if (option === "on") {
    antiStickerData[jid] = true;
    botSettings.update(botJid, s => { s.groups.antisticker = antiStickerData; return s; });
    return replyWithContact(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑬𝑵𝑨𝑩𝑳𝑬𝑫

✨ 𝑺𝒕𝒊𝒄𝒌𝒆𝒓𝒔 𝒘𝒊𝒍𝒍 𝒃𝒆 𝒂𝒖𝒕𝒐𝒎𝒂𝒕𝒊𝒄𝒂𝒍𝒍𝒚 𝒅𝒆𝒍𝒆𝒕𝒆𝒅 🗑️
📍 𝑨𝒄𝒕𝒊𝒗𝒆 𝒊𝒏 𝒕𝒉𝒊𝒔 𝒈𝒓𝒐𝒖𝒑

╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`);
  }

  if (option === "off") {
    delete antiStickerData[jid];
    botSettings.update(botJid, s => { s.groups.antisticker = antiStickerData; return s; });
    return replyWithContact(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑫𝑰𝑺𝑨𝑩𝑳𝑬𝑫

✅ 𝑺𝒕𝒊𝒄𝒌𝒆𝒓𝒔 𝒂𝒓𝒆 𝒏𝒐𝒘 𝒂𝒍𝒍𝒐𝒘𝒆𝒅
📍 𝑷𝒓𝒐𝒕𝒆𝒄𝒕𝒊𝒐𝒏 𝒊𝒔 𝒏𝒐𝒘 𝒐𝒇𝒇

╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`);
  }

  if (option === "list") {
    const groups = Object.keys(antiStickerData).filter(g => antiStickerData[g] === true);
    if (!groups.length) {
      return replyWithContact(`🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑳𝑰𝑺𝑻

❌ 𝑵𝒐 𝒈𝒓𝒐𝒖𝒑𝒔 𝒉𝒂𝒗𝒆 𝑨𝒏𝒕𝒊𝑺𝒕𝒊𝒄𝒌𝒆𝒓 𝒆𝒏𝒂𝒃𝒍𝒆𝒅.

╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`);
    }

    let out = `🛡️ 𝑨𝑵𝑻𝑰 𝑺𝑻𝑰𝑪𝑲𝑬𝑹 𝑳𝑰𝑺𝑻
...
`;
    for (let i = 0; i < groups.length; i++) {
      try {
        const metadata = await sock.groupMetadata(groups[i]);
        out += `${i + 1}️⃣ ${metadata.subject}
`;
      } catch {
        out += `${i + 1}️⃣ Unknown Group
`;
      }
    }
    out += `
📊 𝑻𝒐𝒕𝒂𝒍: ${groups.length} 𝑮𝒓𝒐𝒖𝒑𝒔

╰➤ 🤖 𝑿 𝑵𝑶𝑩𝑰𝑻𝑨 𝑴𝑶𝑫𝒁`;
    return replyWithContact(out);
  }

  return replyWithContact(`❌ 𝑼𝒔𝒆: ${settings.prefix}antisticker on/off/list`);
}
...
case "antigroupmention": {
  ...
  if (!["on", "off"].includes(gmMode))
    return replyWithContact("💡 𝑼𝒔𝒆: .antigroupmention on / .antigroupmention off");

  if (gmMode === "on") {
    botSettings.setGroup(botJid, "antigm", jid, true);
    return replyWithContact("🛡️ 𝑨𝑵𝑻𝑰 𝑮𝑹𝑶𝑼𝑷 𝑴𝑬𝑵𝑻𝑰𝑶𝑵 𝑬𝑵𝑨𝑩𝑳𝑬𝑫\n\n👁️ 𝑨𝒄𝒄𝒆𝒔𝒔𝒊𝒃𝒍𝒆 𝒈𝒓𝒐𝒖𝒑-𝒎𝒆𝒏𝒕𝒊𝒐𝒏 𝒔𝒕𝒂𝒕𝒖𝒔 𝒆𝒗𝒆𝒏𝒕𝒔 𝒘𝒊𝒍𝒍 𝒃𝒆 𝒎𝒐𝒏𝒊𝒕𝒐𝒓𝒆𝒅.");
  }

  botSettings.setGroup(botJid, "antigm", jid, undefined);
  return replyWithContact("🛡️ 𝑨𝑵𝑻𝑰 𝑮𝑹𝑶𝑼𝑷 𝑴𝑬𝑵𝑻𝑰𝑶𝑵 𝑫𝑰𝑺𝑨𝑩𝑳𝑬𝑫");
}

case "antilink":

case "antilinkmode":
  if (!isGroup) return replyWithContact("🚫 ɢʀᴏᴜᴘs ᴏɴʟʏ");
  if (!isAdmin) return replyWithContact("🚫 ᴏɴʟʏ ɢʀᴏᴜᴘ ᴀᴅᴍɪɴs ᴄᴀɴ ᴜsᴇ ᴛʜɪs");

  const alMode = args[0]?.toLowerCase();

  if (!alMode) {
    return replyWithContact("⚙️ ᴀɴᴛɪʟɪɴᴋ • ᴡᴀʀɴ | ᴅᴇʟᴇᴛᴇ | ᴋɪᴄᴋ | ᴏғғ");
  }

  if (alMode === "off") {
    botSettings.setGroup(botJid, "antilink", jid, undefined);
    return replyWithContact("❌ ᴀɴᴛɪʟɪɴᴋ ᴅɪsᴀʙʟᴇᴅ");
  }

  if (!["warn", "delete", "kick"].includes(alMode))
    return replyWithContact("❌ ɪɴᴠᴀʟɪᴅ ᴀɴᴛɪʟɪɴᴋ ᴍᴏᴅᴇ");

  botSettings.setGroup(botJid, "antilink", jid, alMode);

  if (alMode === "warn")
    return replyWithContact("✅ ᴀɴᴛɪʟɪɴᴋ ᴡᴀʀɴ ᴇɴᴀʙʟᴇᴅ • ᴡᴀʀɴ ③/③ = ᴋɪᴄᴋ");

  if (alMode === "delete")
    return replyWithContact("✅ ᴀɴᴛɪʟɪɴᴋ ᴅᴇʟᴇᴛᴇ ᴇɴᴀʙʟᴇᴅ");

  if (alMode === "kick")
    return replyWithContact("✅ ᴀɴᴛɪʟɪɴᴋ ᴋɪᴄᴋ ᴇɴᴀʙʟᴇᴅ");
  break;
...
case "welcome": {
  ...
  if (!wOption || !["on", "off"].includes(wOption))
    return replyWithContact("🌸 𝙒𝙚𝙡𝙘𝙤𝙢𝙚\n💗 .welcome on\n💗 .welcome off");

  const wData = botSettings.group(botJid, "welcome");
  wData[jid] = wOption === "on";
  botSettings.update(botJid, s => { s.groups.welcome = wData; return s; });
...
case "goodbye": {
  ...
  if (!gOption || !["on", "off"].includes(gOption))
    return replyWithContact("🌸 𝙂𝙤𝙤𝙙𝙗𝙮𝙚\n💗 .goodbye on\n💗 .goodbye off");

  const gData = botSettings.group(botJid, "goodbye");
  gData[jid] = gOption === "on";
  botSettings.update(botJid, s => { s.groups.goodbye = gData; return s; });
...
case "gstatus": {
  if (!isGroup) return replyWithContact("❌ 𝙂𝙧𝙤𝙪𝙥 𝙤𝙣𝙡𝙮!");
  if (!isOwner) return replyWithContact("❌ 𝙊𝙬𝙣𝙚𝙧 𝙤𝙣𝙡𝙮!");
  ...
  try {
    ...
    if (!storyData) {
      ...
      return replyWithContact("❌ 𝙐𝙣𝙨𝙪𝙥𝙥𝙤𝙧𝙩𝙚𝙙 𝙢𝙚𝙙𝙞𝙖.");
    }
    ...
    await replyWithContact("🌸 𝙂𝙧𝙤𝙪𝙥 𝙎𝙩𝙖𝙩𝙪𝙨 𝙎𝙚𝙣𝙩 💗");
    ...
    await replyWithContact(`❌ 𝙂𝙧𝙤𝙪𝙥 𝙎𝙩𝙖𝙩𝙪𝙨 𝙁𝙖𝙞𝙡𝙚𝙙\n💡 ${error?.message || error}`);
  }
  break;
}
        ...
        case "wstatus":
          ...
          const wsData = botSettings.group(botJid, "welcome");
          if (wsData[jid] === true) {
            await replyWithContact(`╭━━━〔 ✦ 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 ✦ 〕━━━╮\n┃\n┃ 🟢 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 𝐒𝐓𝐀𝐓𝐔𝐒 : 𝐎𝐍\n┃ ✅ এই group-এ welcome active\n┃\n┃ 🖤 𝙭 𝙣σвιтα\n╰━━━━━━━━━━━━━━━━━━━━╯`);
          } else {
        ...
        case "warn":
          if (!isGroup) return replyWithContact("❌ This command only works in groups!");
          if (!isAdmin && !isOwner) return replyWithContact("❌ Admin or Owner only!");
          const mentionW = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
          if (!mentionW) return replyWithContact("❌ Please mention the user.\n\nExample: .warn @user");
          const metaW = await sock.groupMetadata(jid);
          const botNum = sock.user.id.split(":")[0] + "@s.whatsapp.net";
          if (metaW.participants.find(p => p.id === mentionW)?.admin) return replyWithContact("❌ You cannot warn a group admin.");
          if (mentionW === botNum) return replyWithContact("❌ I cannot warn myself.");
          const warns = botSettings.group(botJid, "warnings");
          if (!warns[jid]) warns[jid] = {};
          warns[jid][mentionW] = (warns[jid][mentionW] || 0) + 1;
          const countW = warns[jid][mentionW];
          if (countW >= 3) {
            delete warns[jid][mentionW];
            botSettings.update(botJid, s => { s.groups.warnings = warns; return s; });
            await replyWithContact(`╭━━━〔 ✦ 𝐖𝐀𝐑𝐍𝐈𝐍𝐆 ✦ 〕━━━╮\n┃\n┃ 👤 𝐔𝐒𝐄𝐑 : @${mentionW.split("@")[0]}\n┃ ⚠️ 𝐖𝐀𝐑𝐍 : 𝟑/𝟑\n┃\n┃ 🚫 𝐌𝐀𝐗 𝐖𝐀𝐑𝐍 𝐑𝐄𝐀𝐂𝐇𝐄𝐃\n┃ 👢 𝐔𝐒𝐄𝐑 𝐑𝐄𝐌𝐎𝐕𝐄𝐃\n┃\n┃ 🖤 𝙭 𝙣σвιтα\n╰━━━━━━━━━━━━━━━━━━━━╯`, { mentions: [mentionW] });
        ...
        case "warnlist":
          if (!isGroup) return replyWithContact("❌ This command only works in groups!");
          const allWarns = botSettings.group(botJid, "warnings");
          const groupWarns = allWarns[jid] || {};
          const warnUsers = Object.entries(groupWarns);
          if (!warnUsers.length) return replyWithContact(`╭━━━〔 ✦ 𝐖𝐀𝐑𝐍 𝐋𝐈𝐒𝐓 ✦ 〕━━━╮\n┃\n┃ ✅ No active warnings.\n┃\n╰━━━━━━━━━━━━━━━━━━━━╯`);
          ...
          warnText += `╰━━━━━━━━━━━━━━━━━━━━╯`;
          await replyWithContact(warnText, { mentions: warnMentions });
          break;
        ...
        case "resetwarn":
          ...
          if (!mentionR) return replyWithContact("❌ Mention a user.\n\nExample: .resetwarn @user");
          const rwData = botSettings.group(botJid, "warnings");
          if (rwData[jid] && rwData[jid][mentionR]) {
            delete rwData[jid][mentionR];
            botSettings.update(botJid, s => { s.groups.warnings = rwData; return s; });
          }
          await replyWithContact(`╭━━━〔 ✦ 𝐖𝐀𝐑𝐍 RESET ✦ 〕━━━╮\n┃\n┃ 👤 @${mentionR.split("@")[0]}\n┃ ✅ Warning count reset.\n┃\n┃ 🖤 𝙭 𝙣σвιтα\n╰━━━━━━━━━━━━━━━━━━━━╯`, { mentions: [mentionR] });
          break;

        case "play": {
  if (!q) {
    return replyWithContact("🎵 Example: .play Believer");
  }

  await replyWithContact("🔍 Searching song...");

  const search = await yts(q);
  const video = search.videos?.[0];

  if (!video) {
    return replyWithContact("❌ No results found.");
  }

  await replyWithContact(`⏳ Downloading...\n🎵 ${video.title}`);
  ...
  break;
        ...
        case "vv": {
          ...
          if (!quotedMessage) {
            await replyWithContact("❌ *Reply to a View Once message first.*");
            break;
          }
          ...
          try {
            ...
            if (!mediaMsg) {
              await replyWithContact("❌ *This is not a supported View Once media message.*");
              break;
            }
            ...
            await replyWithContact(`❌ *View Once retrieval failed.*\n\`${error?.message || error}\``);
          }
          break;
        }

        case "sticker":
          await replyWithContact("Sticker conversion is not implemented yet.");
          break;
...
╰━━━━━━━━━━━━━━━━━━━━╯`);
        ...
        case "truth":
          await replyWithContact(`💭 𝑻𝒓𝒖𝒕𝒉\n${randomPick(truths)}\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "dare":
          await replyWithContact(`🔥 𝑫𝒂𝒓𝒆\n${randomPick(dares)}\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "riddle":
          await replyWithContact(`🧩 𝑹𝒊𝒅𝒅𝒍𝒆\n${randomPick(riddles)}\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "fact":
          await replyWithContact(`🌟 𝑭𝒂𝒄𝒕\n${randomPick(facts)}\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "8ball":
          await replyWithContact(`🎱 𝟴𝑩𝒂𝒍𝒍\n${randomPick(ballAnswers)}\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "dice":
          await replyWithContact(`🎲 𝑫𝒊𝒄𝒆\n🎯 𝑹𝒆𝒔𝒖𝒍𝒕 : ${Math.floor(Math.random() * 6) + 1}\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "coin":
          await replyWithContact(`🪙 𝑪𝒐𝒊𝒏\n${Math.random() < 0.5 ? "𝑯𝒆𝒂𝒅𝒔 🪙" : "𝑻𝒂𝒊𝒍𝒔 🪙"}\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "rate":
          await replyWithContact(`⭐ 𝑹𝒂𝒕𝒆\n𝑹𝒂𝒕𝒊𝒏𝒈 : ${Math.floor(Math.random() * 101)}%\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "howlucky":
          await replyWithContact(`🍀 𝑳𝒖𝒄𝒌\n𝑳𝒖𝒄𝒌 : ${Math.floor(Math.random() * 101)}%\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "howcute":
          await replyWithContact(`💖 𝑪𝒖𝒕𝒆\n𝑪𝒖𝒕𝒆𝒏𝒆𝒔𝒔 : ${Math.floor(Math.random() * 101)}%\n\n✦ 𝙭 𝙣σвιтα ✦`);
          break;

        case "autoreact":
          await autoreactCommand(args, replyWithContact, isOwner, botJid);
          break;

        case "autotyping":
          await automationCommand("autotyping", args, replyWithContact, isOwner, botJid);
          break;

        case "autoread":
          await automationCommand("autoread", args, replyWithContact, isOwner, botJid);
          break;

        case "autostoryview":
          await automationCommand("autostoryview", args, replyWithContact, isOwner, botJid);
          break;

        case "anime":
          await sendAnime(sock, jid, msg, replyWithContact, q, "search");
          break;

        case "animesearch":
          await animeSearch(replyWithContact, q);
          break;

        case "animeinfo":
          await sendAnime(sock, jid, msg, replyWithContact, q, "info");
          break;

        case "manga":
          await manga(replyWithContact, q);
          break;

        case "character":
          await character(replyWithContact, q);
          break;

        case "waifu":
          await sendRandomImage(sock, jid, msg, replyWithContact, "waifu");
          break;
        ...
        case "husband":
          await sendRandomImage(sock, jid, msg, replyWithContact, "husband");
          break;

        case "neko":
          await sendRandomImage(sock, jid, msg, replyWithContact, "neko");
          break;

        case "animefact":
          await animeFact(replyWithContact);
          break;
  ...
  sock.ev.on("messages.upsert", async (data) => {
    for (const msg of data.messages) {
      ...
      await handleAutomation(sock, msg, (sock.user?.id || options.pairedPhone || "").split(":")[0]);
      await processCommandMessage(msg);
      await handleAutoReact(sock, msg, (sock.user?.id || options.pairedPhone || "").split(":")[0]);
    }
  });
