...
function startTelegramPairing() {
  ...
  bot.onText(/^\/pair(?:\s+(.+))?$/i, async (msg, match) => {
    // Pairing is intentionally available only inside Telegram groups.
    // The pairing code is sent to msg.chat.id, so in a group it stays in the group.
    const isGroup = msg.chat?.type === "group" || msg.chat?.type === "supergroup";
    if (!isGroup) {
      await bot.sendMessage(
        msg.chat.id,
        "🚫 <b>GROUP ONLY</b>\n\nPairing is available only inside a Telegram group.\n\n➜ Add this bot to your group and use:\n<code>/pair 917074420859</code>",
        { parse_mode: "HTML" }
      );
      return;
    }
